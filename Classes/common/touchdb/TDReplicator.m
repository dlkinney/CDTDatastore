...
- (void) databaseWasDeleted: (NSNotification*)n {
    ...
    [self queue:^{
        ...
        //should we even check this??  especially if another processes
        //has deleted it. what if our local, weak reference is now nil?
        ...
        self.error = [NSError errorWithDomain:TDInternalErrorDomain
                                     ...
                                     userInfo:userInfo];
        CDTLogInfo(CDTREPLICATION_LOG_CONTEXT, @"During replication %@. databaseWasDeleted block. setting error: %@", self, self.error);
        [self stop];
    }];
}
